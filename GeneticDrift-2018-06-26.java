...
public class ComplexAutomata5 implements Runnable{
	
	//final static boolean wrapped = true;
	final static int width = 256;
	final static int height = 192;
...
//		world = WorldGenerator.addRandomCell(world,Color.BLUE,(int)(width/2),(int)((height-height/5)/2));
		...
		t1 = new Thread(new ComplexAutomata5());
		t1.start();
...
class Cell {
		...
		public Cell makeBaby(Cell cell) {
			...
			if(f < 0.1) { 
				if(r.nextInt(2)==1) {
					c = cell.getColor().darker();
				} else {
					c = cell.getColor().brighter();
				}
			} else {
				c = cell.getColor();
			}*/
